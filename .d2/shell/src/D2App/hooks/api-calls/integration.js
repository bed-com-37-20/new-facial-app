...
export async function registerStudent(form) {
  ...
  const payload = {
    ...
    attributes: [{
      ...
      value: formData.get('academicYear')
    }, {
      attribute: "EHTfWCHTYCo",
      value: formData.get('yearOfStudy')
    }, {
      attribute: "ADiCfoRxZI2",
      value: formData.get('programOfStudy')
    }, {
      attribute: "ixauprApakv",
      value: formData.get('enrollmentDate')
    }, {
      attribute: "ED1V1bFMtb1",
      value: formData.get('profilePictureInput')
    }, {
      ...
      value: formData.get('Nationality')
    }, {
      attribute: "pzZJIX2yMEZ",
      value: formData.get('guardian')
    }, {
      attribute: "ofiRHvsg4Mt",
      value: formData.get('regNumber')
    }]
  };
  try {
...
export async function recordAttendance() {
  ...
  const payload = {
    ...
    dataValues: [{
      dataElement: "NRXM4M9Bkas",
      // Example data element for attendance status
      value: "Yes" // Marking attendance as "Yes"
    }, {
      dataElement: "zEwd7PwSHV9",
      // Example data element for event date
